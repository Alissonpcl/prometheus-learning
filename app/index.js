...
// It is a good practice to use the application name as the metric name prefix
...
const tempoDeResposta = new promClient.Histogram({
	...
	// If a request respond in less than 0.1s, it will count in all bucket.
    // If a request respond in more than 0.1s, it will count in 0.2 bucket and all other after that.
    // +Inf is a bucket that is always present and will always match the count of requests tracked.
    // It contains all values, including the ones greater than 0.5.
    // buckets: [0.1, .2, .3, .4 , .5], // +Inf will be added automatically
});
